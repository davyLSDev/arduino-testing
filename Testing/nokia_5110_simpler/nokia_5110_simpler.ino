...
void loop(void) {
  ...
  LCDClear();
  LCDString("a b c `defghijklmnopqrstuvwxyz 123456789 ABCDEFGHIJKLMNOPQRSTUVWXYZ");
  delay(3000);
}
...
void LCDBitmap(char my_array[]){
  for (int index = 0 ; index < (LCD_X * LCD_Y / 8) ; index++)
    LCDWrite(LCD_DATA+1, my_array[index]);
}
...
void LCDCharacter(char character) {
  ...
  for (int index = 0 ; index < 5 ; index++)
    LCDWrite(LCD_DATA, ASCII[character - 0x21][index]);
